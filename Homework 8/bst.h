...
private:
    ...
    //added a helper funtion for implementing operator==
    bool equals( TreeNode<T>* param1, TreeNode<T>* param2 ) const;
...
bool BSTree<T>::operator==( const BSTree<T>& param1 ) const {
    return equals(root, param1.root);//calls helper function
}

//a recursive helper function implemented for operator== (private member function)
//takes 2 parameters - the roots of two binary search trees and compares the two BSTs associated with the two roots
template <typename T>
bool BSTree<T>::equals( TreeNode<T>* param1, TreeNode<T>* param2 ) const {
    //if both root nodes are NULL both bst's are NULL and hence equal
    if (param1 == NULL && param2 == NULL)
        return true;
    //if one root node is NULL and the other is not one bst is NULL while the other is not hence not equal
    else if (param1 == NULL || param2 == NULL)
        return false;
    //returns bool value of whether root nodes are equal and subsequently, the left and right subtrees are equal (recursively)
    else
        return ((param1->data == param2->data)
                && (equals(param1->left, param2->left))
                && (equals(param1->right, param2->right)));
}

template <typename T>
void BSTree<T>::to_vector( TreeNode<T>* param1 , vector<T>& param2 ) const {
    //if the root node is not NULL, its value is pushed at the back of the vector
    if (param1 != NULL)
        param2.push_back(param1->data);
    //if the left child of the root node is not NULL, the left subtree is pushed at the back of the vector recursively
    if (param1->left != NULL)
        to_vector(param1->left, param2);
    //if the right child of the root node is not NULL, the right subtree is pushed at the back of the vector recursively
...
ostream& operator<<( ostream& param1 , const BSTree<T>& param2 ) {
    vector<T> bst;// a temp vector that stores the contents of bst param2
    param2.to_vector(param2.get_root(), bst);//param2 is converted to a vector
    
    //the contents of vector bst are displayed
